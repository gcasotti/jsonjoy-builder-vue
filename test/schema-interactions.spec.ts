...
describe("Schema Interactions", () => {
    it("should allow changing the type of a property without infinite looping", { timeout: TIMEOUT }, async () => {
        ...
        const { wrapper, store } = mountWithStore(SchemaVisualEditor, { storeSchema: schema });
        ...
        const typeButtons = wrapper.findAll('button').filter(b => b.text().includes('Object'));
        ...
        const optionButtons = wrapper.findAll('button').filter(b => b.text().includes('Text') || b.text().includes('String'));
