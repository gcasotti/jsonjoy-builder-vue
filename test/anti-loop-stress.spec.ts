...
import { createSchemaStore, SchemaStoreKey } from "../src/hooks/useSchemaStore.ts";
...
describe("Anti-Loop Stress Test", () => {
    it("should handle 20 rapid type changes without hanging", { timeout: 10000 }, async () => {
        const schema: JSONSchema = {
            type: "object",
            properties: {
                field1: { type: "string" },
                field2: { type: "number" },
                field3: { type: "object", properties: { nested: { type: "string" } } },
            },
        };
        const { store } = mountWithStore(SchemaVisualEditor, schema);

        // Rapidly toggle between types 20 times
        const types = ["string", "number", "boolean", "object", "array", "string"] as const;
        for (let i = 0; i < 20; i++) {
    ...
    it("should handle 10 rapid add+delete operations without hanging", { timeout: 10000 }, async () => {
        const schema: JSONSchema = {
            type: "object",
            properties: {},
        };
        ...
        for (let i = 0; i < 10; i++) {
            store.addProperty([], { name: `tmp${i}`, type: "string", description: "", required: false });
            await nextTick();
            store.deleteProperty([], `tmp${i}`);
            await nextTick();
        }

        expect(Object.keys((store.schema.value as any).properties || {})).toHaveLength(0);
    });

    it("should handle nested schema modifications without looping", { timeout: 10000 }, async () => {
        const schema: JSONSchema = {
            type: "object",
            properties: {
                level1: {
        ...
        expect((store.schema.value as any).properties.level1.type).toBe("boolean");
    });
});
